...
Lista *constroiLista(void){
      ...
      l->inicio = l->fim = NULL; // CORRIGE OS PONTEIROS
      return l;	// RETORNA A LISTA CRIADA
}

/*** INSERE UM NO NO FINAL LISTA ***/
No *insereNo(Lista *l, No n){  
     if(l->tam == 0){
                   l->inicio = (No *)malloc(sizeof(No)); 		// Aloca espaco
                   l->fim = l->inicio;							// l->fim recebe o mesmo endereço de l->inicio
                   *(l->inicio) = n;                 			// Recebe no
                   l->fim->prox = NULL;              			// Coloca nulo no fim da lista
                   l->inicio->ant = NULL;            			// Coloca nulo no inicio da lista
                   l->tam++;      							// Expande lista             
     }
     else{
                   l->fim->prox = (No *) malloc(sizeof(No));		// Aloca espaco
                   l->fim->prox->ant = l->fim;     				// Ajusta ponteiro do no alocado
                   l->fim = l->fim->prox;     					// l->fim recebe endereço do no alocado
                   *(l->fim) = n;								// Recebe n
                   l->fim->prox = NULL;							// Coloca nulo no fim da lista
                   l->tam++;                                // Expande lista
     }
...
void leArquivo(char *dir, char *arq, Lista *l){
	...
	while (!feof(histogramas)){
		fscanf(histogramas, "%s %d %d %d %d", aux->nome, &aux->max_h, &aux->max_s, &aux->max_v, &aux->erro);
		if (feof(histogramas)) return;
		l->fim = insereNo(l, *aux);
	}
